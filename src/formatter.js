...
function sectorDetailDivider(sector) {
  const s = SECTOR_COLORS[sector] || { border: '#64748b', bg: '#1e293b', text: '#94a3b8' };
  return `<div style="background:${s.bg};border:1px solid ${s.border};border-radius:8px;padding:8px 14px;margin:8px 0 16px">
    <span style="font-size:11px;font-weight:700;color:${s.text};text-transform:uppercase;letter-spacing:.08em">${esc(sector)}</span>
  </div>`;
}
...
<body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;max-width:700px;margin:0 auto;padding:20px;background:#0f172a;color:#f1f5f9">
  ...
  ${ipoSectionHTML(ipos)}
  ${groupBySector(results).flatMap(([sector, tickers]) => [sectorDetailDivider(sector), ...tickers.map(r => detailCard(r, benchmark))]).join('')}
